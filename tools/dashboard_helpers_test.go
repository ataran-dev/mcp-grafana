...
func TestExtractPanelQueries(t *testing.T) {
	t.Run("extracts prometheus queries", func(t *testing.T) {
		...
		assert.Equal(t, "rate(http_requests_total{job=\"$job\"}[5m])", queries[0].RawQuery)
		assert.Equal(t, "rate(http_requests_total{job=\"api-server\"}[5m])", queries[0].ProcessedQuery)
		assert.Equal(t, "prometheus", queries[0].DatasourceUID)
		assert.Equal(t, "prometheus", queries[0].DatasourceType)
	...
	t.Run("uses target-level datasource when available", func(t *testing.T) {
		...
		require.Len(t, queries, 1)
		assert.Equal(t, "target-ds", queries[0].DatasourceUID)
		assert.Equal(t, "loki", queries[0].DatasourceType)
	})
	...
	t.Run("substitutes datasource variable", func(t *testing.T) {
		...
		require.Len(t, queries, 1)
		assert.Equal(t, "prometheus-prod", queries[0].DatasourceUID)
	})
